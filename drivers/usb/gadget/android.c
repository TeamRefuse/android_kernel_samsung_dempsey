...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	if (id < 0)
		return id;
// HDLNC_OPK_20110325: For device serial infomation [ cmd : adb devices ]		
	get_usb_serial(usb_serial_number);
	strings_dev[STRING_SERIAL_IDX].id = id;
	device_desc.iSerialNumber = id;

	if( (usb_serial_number[0] + usb_serial_number[1] + usb_serial_number[2]) != 0 )
		strcpy((char *)(strings_dev[STRING_SERIAL_IDX].s), usb_serial_number);
	CSY_DBG2("[ADB_UMS_ACM_RNDIS_MTP] string_dev = %s \n",strings_dev[STRING_SERIAL_IDX].s);
// HDLNC_OPK_20110325
...
static DEVICE_ATTR(tethering, S_IRUGO |S_IWGRP | S_IRUSR | S_IWUSR, tethering_switch_show, tethering_switch_store);
