...
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
static int s5k6aafx_check_sensorId(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err = 0;
	unsigned short read_value;
	if (gcamera_sensor_front_checked) 
	{
		...
		if (gcamera_sensor_front_type == CAMERA_SENSOR_ID_FRONT_S5K6AAFX)
			return 0;
		else
			return -EIO;
	}
	err = s5k6aafx_read(client, 0x0134, &read_value);
	if (read_value==0x734f)
	{ 
		gcamera_sensor_front_type = CAMERA_SENSOR_ID_FRONT_S5K6AAFX;
		gcamera_sensor_front_checked = true;
	}
	printk("%s: (%d)(%d)\n", __func__ , err ,gcamera_sensor_front_type);
	return err;
} 
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
...
static int s5k6aafx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (!gcamera_sensor_front_checked) //NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
		return -EINVAL;
...
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera	
	...
	switch (ctrl->id) 
	{
		case V4L2_CID_CAM_FRONT_SENSOR_TYPE:
			err = s5k6aafx_check_sensorId(sd);
			break;
		default:
